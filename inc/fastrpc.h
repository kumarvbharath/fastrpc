...
/**
 * @brief Callback function type for FastRPC events.
 *
 * @param type The type of callback event.
 * @param context A pointer to the context data associated with the callback.
 * @param data A pointer to the data associated with the callback.
 * @param result A pointer to a void pointer where the callback can store a result.
 *
 * @return int An error code indicating the success or failure of the callback.
 */
typedef int (*fastrpc_callback_t)(int type, void **context, void *data, void **result);
