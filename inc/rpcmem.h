...
struct dsp_mem {
    ...
    QList allocations;       /**< List of active allocations */
};

// Update struct to include fd handling
struct mem_ops {
    void* (*alloc)(size_t size, uint32_t flags, int *fd);
    void (*free)(void* ptr, int fd, size_t size);
};
