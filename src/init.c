...
/* Static module registration lock */
static pthread_mutex_t static_modules_lock = PTHREAD_MUTEX_INITIALIZER;
...
void fastrpc_default_configurations() {
    ...
    LOG_INF("flags %p, attr %p", &flags, &attr);
}
...
void fastrpc_other_init(void) {
    ...
    pthread_mutex_init(&static_modules_lock, &attr);
...
void register_static_modules(struct session *sess) {
    LOG_INF("Registering static modules for FastRPC session");
    pthread_mutex_lock(&static_modules_lock);
    if(!g_modules_initialized) {
        LOG_INF("Registering static modules for FastRPC session");
        session_register_module(sess, "remotectl", REMOTECTL_HANDLE);
        g_modules_initialized = 1;
        LOG_INF("Static modules registered successfully");
    }
