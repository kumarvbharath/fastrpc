...
static int call_dsp_callbacks(int callback_type, void *data) {
    ...
    if (QList_IsEmpty(&dsp_callbacks)) {
        LOG_INF("No DSP callbacks registered");
        return AEE_EFAILED;
    }
    ...
    QLIST_NEXTSAFE_FOR_ALL(&dsp_callbacks, node, nnode) {
        ...
        if (callback_node->type & callback_type) {
            err = callback_node->callback(callback_type, &callback_node->context, data, &ret);
            if (err) {
                return err;
            }
        }
...
static int call_session_callbacks(struct dsp *dsp, int callback_type, void *data, void **result) {
    QNode *node, *nnode;
    void *cbresult;
    int err = AEE_SUCCESS;
    ...
    QLIST_NEXTSAFE_FOR_ALL(&dsp->session_callbacks, node, nnode) {
        ...
        if (callback_node->type & callback_type) {
            err = callback_node->callback(callback_type, &callback_node->context, data, &cbresult);
...
struct session *get_session_from_handle(remote_handle64 handle)
{
    ...
    if (entry) {
        ...
        LOG_ERR("Handle %lx not found in hash table", handle);
        return NULL;
    }
}

/* Initialize FastRPC */
static void fastrpc_core_init_impl(void)
{
...
struct dsp *dsp_init(int dsp_id)
{
    ...
    if(dsp_id < 0) {
        LOG_DSP_INIT_ERR("Invalid parameter dspid %d", dsp_id);
        return NULL;
    }
    ...
    if (dsp) {
        dsp->active_dsp++;
        LOG_DSP_INIT_INF("DSP ID %d already initialized, %p, count %d", dsp_id, dsp, dsp->active_dsp);
        unlock_dsp_list();
        return dsp;
    }
    ...
    LOG_DSP_INIT_INF("Initialized DSP ID %d, %p", dsp_id, dsp);
    unlock_dsp_list();
...
struct session *session_init(struct dsp *dsp, int session_id)
{
    ...
    if(session_id < 0) {
        LOG_SESS_INIT_ERR("Invalid session id");
        return NULL;
    }
    ...
    int err = call_session_callbacks(dsp, CALLBACK_TYPE_INIT, sess, NULL);
    if (err) {
...
int session_deinit(struct session *sess)
{
    ...
    QLIST_NEXTSAFE_FOR_ALL(&dsp_list, node, nnode) {
        ...
        lock_dsp(dsp);
        if (QList_IsEmpty(&dsp->sessions)) {
            LOG_SESS_DEINIT_INF("No sessions found in DSP ID %d, moving to the next dsp", dsp->dsp_id);
            unlock_dsp(dsp);
            continue;
        }
        ...
        int err = call_session_callbacks(dsp, CALLBACK_TYPE_DEINIT, sess, NULL);
        if (err) {
            unlock_dsp(dsp);
            unlock_dsp_list();
...
remote_handle64 session_add_module(struct session *sess, const char *name)
{
    ...
    if (!sess) {
        LOG_SESS_ADDMOD_ERR("Invalid session pointer");
    ...
    if (!mod) {
        LOG_SESS_ADDMOD_ERR("Failed to allocate memory for module %s", name);
        return INVALID_HANDLE;
    }
    ...
    err = call_session_callbacks(dsp, CALLBACK_TYPE_LOAD, mod->name, &h);
    if (err) {
        LOG_SESS_ADDMOD_ERR("Failed to load module %s", mod->name);
        FREE(mod);
        return INVALID_HANDLE;
    }

    mod->rhandle = (remote_handle)h;
    ...
    LOG_SESS_ADDMOD_INF("Added module %s to session ID %d", name, sess->session_id);
    return mod->handle;
}

/* Remove Module from Session */
int session_remove_module(struct session *sess, remote_handle64 handle)
{
    ...
    if (!sess) {
        LOG_SESS_RMMOD_ERR("Invalid session pointer");
        return AEE_EINVALIDPARAM;
    }

    if(handle == INVALID_HANDLE) {
        LOG_SESS_RMMOD_ERR("Invalid handle");
        return AEE_EINVALIDPARAM;
    }
    ...
    if(mod) {
        err = call_session_callbacks(dsp, CALLBACK_TYPE_UNLOAD, sess, NULL);
        if(err) {
            LOG_SESS_RMMOD_ERR("Failed to unload module ID %p", (void*)handle);
        }
        ...
        if(HASH_COUNT(handles) <= 0) {
            err = call_session_callbacks(dsp, CALLBACK_TYPE_CLOSE, sess, NULL);
            if (err) {
                LOG_SESS_RMMOD_ERR("Failed in close callback for session %d", sess->session_id);
            }
        }
        pthread_spin_lock(&sess->lock);
        sess->active_users--;
        ...
        LOG_SESS_RMMOD_INF("Removed module ID %p from session ID %d", (void*)handle, sess->session_id);
    } else {
        LOG_SESS_RMMOD_ERR("Module ID %p not found in session ID %d", (void*)handle, sess->session_id);
    }
    return err;
}

/* Configure Session */
int session_configure(struct session *sess, char* key, void *value, size_t size)
{
    int err;
    struct dsp *dsp = NULL;
    ...
    if (!key || !value) {
        LOG_SESS_CFG_ERR("Invalid config key or value");
        return AEE_EINVALIDPARAM;
    }
    if (!sess) {
        LOG_SESS_CFG_ERR("Invalid session pointer");
        return AEE_EINVALIDPARAM;
    }
    ...
    LOG_SESS_CFG_INF("Configured session ID %d with config key %s", sess->session_id, key);
    return AEE_SUCCESS;
}
...
int session_invoke(struct session *sess, remote_handle64 handle, uint32_t sc, remote_arg *params)
{
    ...
    err = call_session_callbacks(dsp, CALLBACK_TYPE_INVOKE, &invoke_args, NULL);
    if(err) {
        LOG_SESS_INV_ERR("Failed to invoke session ID %d with handle %lx and sc %u", 
                 sess->session_id, handle, sc);
        return AEE_EFAILED;
    }

    // Add your invocation logic here
    LOG_SESS_INV_INF("Invoked session ID %d with handle %lx and sc %u", sess->session_id, handle, sc);
    return AEE_SUCCESS;
}
...
void *session_map_memory(struct session *sess, struct rpcmem *mem)
{
    ...
    int err = call_session_callbacks(dsp, CALLBACK_TYPE_MAP, mem_copy, NULL);
    if (err) {
...
int session_unmap_memory(struct session *sess, struct rpcmem *mem)
{
    int err;
    struct dsp *dsp = NULL; // Assume you have a way to get the DSP from the session
    QNode *node;
    ...
    err = call_session_callbacks(dsp, CALLBACK_TYPE_UNMAP, mem, NULL);
    if (err) {
